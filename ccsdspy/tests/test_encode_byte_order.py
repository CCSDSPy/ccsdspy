...
class TestEncodeByteOrderFixedLength:
    ...
    def test_encode_big_endian_uint(self):
        ...
        try:
            packet.to_file(
                temp_file, pkt_type=0, apid=100, sec_header_flag=0, seq_flag=0, data=data
            )
    ...
    def test_encode_array_little_endian(self):
        ...
        packet = FixedLength(
            [
                PacketArray(
                    name="data", data_type="uint", bit_length=16, array_shape=4, byte_order="little"
                )
            ]
        )
        ...
        # Create test data
        data = {
            "data": np.array([[0x1234, 0x5678, 0x9ABC, 0xDEF0], [0xAAAA, 0xBBBB, 0xCCCC, 0xDDDD]])
        }
    ...
    def test_encode_multiple_fields_mixed_byte_order(self):
        ...
        packet = FixedLength(
            [
                ...
                PacketField(
                    name="little_val", data_type="uint", bit_length=32, byte_order="little"
                ),
